						...
						<div class="box-body">
							<div class="form-group <?=form_error('nama') ? 'has-error' : ''?>">
							 	<label for='nama'>Nama Kategori</label> <span class="text-red">*</span>
							  	<input type="text" class="form-control" id='nama' name='nama' value="<?=set_value('nama', $kategori_buku->nama)?>" placeholder="Enter name">
							  	<?=form_error('nama')?>
							</div>
							<div class="form-group <?=form_error('deskripsi') ? 'has-error' : ''?>">
							  	<label for='deskripsi'>Deskripsi</label> <span class="text-red">*</span>
							  	<textarea name='deskripsi' id='deskripsi' cols="30" rows="5" class="form-control" placeholder="Enter description"><?=set_value('deskripsi', $kategori_buku->deskripsi)?></textarea>
							...
							<div class="form-group <?=form_error('coverphoto') ? 'has-error' : ''?>">
						        ...
						        <div class="input-group">
						        	<img class="userprofileimg" src="<?=app_image_link($kategori_buku->coverphoto,'uploads/kategori_buku/','kategori_buku.jpg')?>" alt="">
						        </div>
							...
							<div class="form-group <?=form_error('status') ? 'has-error' : ''?>">
							  	...
							  	<?php 
									...
									echo form_dropdown('status', $statusArray, set_value('status', $kategori_buku->status),'id='status' class="form-control"');
								?>
							  	<?=form_error('status')?>
							</div>
						</div>
