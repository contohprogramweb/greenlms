                    ...
                    <form peran="form" method="POST">
                        <div class="box-body">
                            <div class="form-group <?=form_error('nama') ? 'has-error' : ''?>">
                                ...
                                <input type="text" class="form-control" value="<?=set_value('nama', $catatan_izin->nama)?>" id='nama' name='nama'/>
                                <?=form_error('nama')?>
                            </div>
                            <div class="form-group <?=form_error('deskripsi') ? 'has-error' : ''?>">
                                <label for='deskripsi'>Deskripsi</label> <span class="text-red">*</span>
                                <textarea name='deskripsi' id='deskripsi' cols="30" rows="3" class="form-control"><?=set_value('deskripsi', $catatan_izin->deskripsi)?></textarea>
                                <?=form_error('deskripsi')?>
                            </div>
                            <div class="form-group <?=form_error('aktif') ? 'has-error' : ''?>">
                                <label for='aktif'>Aksi</label> <span class="text-red">*</span>
                                <input type="text" class="form-control" value="<?=set_value('aktif',$catatan_izin->aktif)?>" id='aktif' name='aktif' />
                                <?=form_error('aktif')?>
                            </div>
                        </div>
