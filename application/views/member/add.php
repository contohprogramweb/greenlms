						...
						<div class="box-body">
							...
							<div class="form-group <?=form_error('jenis_kelamin') ? 'has-error' : ''?>">
							  	...
							  	<?php 
									...
									echo form_dropdown('jenis_kelamin', $genderArray, set_value('jenis_kelamin'),'id='jenis_kelamin' class="form-control"');
								?>
							  	<?=form_error('jenis_kelamin')?>
							</div>
							<div class="form-group <?=form_error('agama') ? 'has-error' : ''?>">
							  	<label for='agama'>Agama</label> <span class="text-red">*</span>
							  	<input type="text" class="form-control" id='agama' name='agama' value="<?=set_value('agama')?>" placeholder="Enter religion">
							  	<?=form_error('agama')?>
							</div>
							<div class="form-group <?=form_error('surel') ? 'has-error' : ''?>">
							  	<label for='surel'>Email</label> <span class="text-red">*</span>
							  	<input type="text" class="form-control" id='surel' name='surel' value="<?=set_value('surel')?>" placeholder="Enter email">
							  	<?=form_error('surel')?>
							</div>
							<div class="form-group <?=form_error('telepon') ? 'has-error' : ''?>">
							  	<label for='telepon'>Nomor Telp.</label> <span class="text-red">*</span>
							  	<input type="text" class="form-control" id='telepon' name='telepon' value="<?=set_value('telepon')?>" placeholder="Enter phone">
							  	<?=form_error('telepon')?>
							</div>
							...
							<div class="form-group <?=form_error('alamat') ? 'has-error' : ''?>">
							  	...
							  	<textarea name='alamat' value="<?=set_value('alamat')?>" id="" cols="30" rows="5" class="form-control" placeholder="Enter address"><?=set_value('alamat')?></textarea>
							  	<?=form_error('alamat')?>
							</div>
							...
							<div class="form-group <?=form_error('nama_pengguna') ? 'has-error' : ''?>">
							  	...
							  	<input type="text" class="form-control" id='nama_pengguna' name='nama_pengguna' value="<?=set_value('nama_pengguna')?>" placeholder="Enter username">
							  	<?=form_error('nama_pengguna')?>
							</div>
							<div class="form-group <?=form_error('kata_sandi') ? 'has-error' : ''?>">
							  	<label for='kata_sandi'>Password</label> <span class="text-red">*</span>
