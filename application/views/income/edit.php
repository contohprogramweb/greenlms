						...
						<div class="box-body">
							<div class="form-group <?=form_error('nama') ? 'has-error' : ''?>">
							 	<label for='nama'>Nama Pendapatan</label> <span class="text-red">*</span>
							  	<input type="text" class="form-control" id='nama' name='nama' value="<?=set_value('nama', $pemasukan->nama)?>" placeholder="Enter Name">
							  	<?=form_error('nama')?>
							</div>
							<div class="form-group <?=form_error('tanggal') ? 'has-error' : ''?>">
								<label for='tanggal'>Tanggal</label> <span class="text-red">*</span>
							  	<input type="text" class="form-control datepicker" id='tanggal' name='tanggal' value="<?=set_value('tanggal', date('d-m-Y', strtotime($pemasukan->tanggal)))?>" placeholder="Enter Date">
							  	<?=form_error('tanggal')?>
							</div>
							<div class="form-group <?=form_error('jumlah') ? 'has-error' : ''?>">
							  	<label for='jumlah'>Jumlah</label> <span class="text-red">*</span>
							  	<input type="text" class="form-control" id='jumlah' name='jumlah' value="<?=set_value('jumlah', $pemasukan->jumlah)?>" placeholder="Enter Amount">
							  	<?=form_error('jumlah')?>
							</div>
							...
							<div class="form-group <?=form_error('catatan') ? 'has-error' : ''?>">
							  	...
							  	<textarea name='catatan' id='catatan' class="form-control" cols="30" rows="5"><?=set_value('catatan', $pemasukan->catatan)?></textarea>
							  	<?=form_error('catatan')?>
							</div>
						</div>
