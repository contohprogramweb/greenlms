...
class Laporantransaksi extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() == false) {
                ...
                $this->data["subview"] = "laporan/transaction/index";
    ...
    public function pdf()
    {
        ...
        if ((int) $reportfor && ((int) $roleID || $roleID == 0) && ((int) $memberID || $memberID == 0) && ((int) $fromdate || $fromdate == 0) && ((int) $todate || $todate == 0)) {
            ...
            $this->pdf->create(['stylesheet' => 'transactionreport.css', 'view' => 'laporan/transaction/pdf.php', 'data' => $this->data]);
    ...
    private function _queryArray($qArray)
    {
        ...
        if (calculate($userArray)) {
            $members = $this->anggota_m->get_order_by_member($userArray);
        }
    ...
    private function _queryArrayDB($queryArray)
    {
        ...
        $roles     = pluck($this->peran_m->get_role(), 'peran', 'id_peran');
        $i         = 1;
        $retArray  = [];
        if ($reportfor == 1) {
            $incomes = $this->pemasukan_m->get_order_by_income_for_report($queryArray);
            ...
            $expenses = $this->pengeluaran_m->get_order_by_expense_for_report($queryArray);
            ...
            $paymentanddiscounts = $this->pembayarandandiskon_m->get_order_by_paymentanddiscount_for_report($queryArray);
            ...
            $finehistorys = $this->riwayatketerlambatan_m->get_order_by_finehistory_for_report($queryArray);
            ...
            $bookissues = $this->peminjamanbuku_m->get_order_by_bookissue_for_report($queryArray);
