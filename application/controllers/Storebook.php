...
class Storebook extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        $this->data['storebooks'] = $this->storebook_m->get_order_by_storebook(['dihapus_pada' => 0]);
    ...
    public function edit()
    {
        ...
        if ((int) $storebookID) {
            $buku_toko = $this->storebook_m->get_single_storebook(array('storebookID' => $storebookID, 'dihapus_pada' => 0));
    ...
    public function view()
    {
        ...
        if ((int) $storebookID) {
            $buku_toko = $this->storebook_m->get_single_storebook(array('storebookID' => $storebookID));
            if (calculate($buku_toko)) {
                $this->data['buku_toko'] = $buku_toko;

                $this->data['kategori_buku_toko'] = [];
                if ((int) $buku_toko->storebookcategoryID) {
                    $this->data['kategori_buku_toko'] = $this->storebookcategory_m->get_single_storebookcategory(['storebookcategoryID' => $buku_toko->storebookcategoryID]);
                }
    ...
    public function delete()
    {
        $storebookID = htmlentities(escapeString($this->uri->segment(3)));
        if ((int) $storebookID) {
            $buku_toko = $this->storebook_m->get_single_storebook(array('storebookID' => $storebookID, 'deleted_at !=' => 1));
            if (calculate($buku_toko)) {
                $this->storebook_m->update_storebook(['dihapus_pada' => 1], $storebookID);
                $this->session->set_flashdata('success', 'Success');
                redirect(base_url('buku_toko/index'));
            } else {
                $this->data["subview"] = "_not_found";
