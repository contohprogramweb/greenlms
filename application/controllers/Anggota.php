...
class Anggota extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        $this->data['members']   = $this->anggota_m->get_order_by_member(['dihapus_pada' => 0, 'id_peran' => $setroleID]);
    ...
    public function add()
    {
        ...
        $this->data['roles'] = $this->peran_m->get_role(array('id_peran', 'peran'));
        if ($_POST) {
            $rules = $this->rules();
            ...
            if ($this->form_validation->run() == false) {
                ...
                $this->anggota_m->insert_member($array);
    ...
    public function edit()
    {
        ...
        if ((int) $memberID) {
            $anggota = $this->anggota_m->get_single_member(array('id_anggota' => $memberID, 'dihapus_pada' => 0));
            if (calculate($anggota)) {
                ...
                if ($_POST) {
                    ...
                    if ($this->form_validation->run() == false) {
                        ...
                        $this->anggota_m->update_member($array, $memberID);
                        $this->session->set_flashdata('success', 'Success');
                        redirect(base_url('anggota/index'));
                    }
                } else {
    ...
    public function view()
    {
        ...
        if ((int) $memberID) {
            $anggota = $this->anggota_m->get_single_member(array('id_anggota' => $memberID));
            if (calculate($anggota)) {
                $this->data['anggota']       = $anggota;
                ...
                $this->data['peran']         = $this->peran_m->get_single_role(array('id_peran' => $anggota->id_peran));
    ...
    public function delete()
    {
        ...
        if ((int) $memberID) {
            $anggota = $this->anggota_m->get_single_member(array('id_anggota' => $memberID));
            if (calculate($anggota)) {
                $this->anggota_m->update_member(['dihapus_pada' => 1], $memberID);
                $this->session->set_flashdata('success', 'Success');
                redirect(base_url('anggota/index'));
            } else {
                $this->data["subview"] = "_not_found";
    ...
    public function check_unique_email($email)
    {
        $memberID = htmlentities(escapeString($this->uri->segment(3)));
        if ((int) $memberID) {
            $anggota = $this->anggota_m->get_single_member(array('surel' => $email, 'memberID !=' => $memberID));
            if (calculate($anggota)) {
            ...
            $anggota = $this->anggota_m->get_single_member(array('surel' => $email));
            if (calculate($anggota)) {
                $this->form_validation->set_message("check_unique_email", "The %s is already exits.");
                return false;
            }
    ...
    public function check_unique_username($username)
    {
        $memberID = htmlentities(escapeString($this->uri->segment(3)));
        if ((int) $memberID) {
            $anggota = $this->anggota_m->get_single_member(array('nama_pengguna' => $username, 'memberID !=' => $memberID));
            if (calculate($anggota)) {
            ...
            $anggota = $this->anggota_m->get_single_member(array('nama_pengguna' => $username));
            if (calculate($anggota)) {
                $this->form_validation->set_message("check_unique_username", "The %s is already exits.");
                return false;
            }
