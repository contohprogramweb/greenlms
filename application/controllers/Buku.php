...
class Buku extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        $this->data['books']   = $this->buku_m->get_order_by_book(['dihapus_pada'=> 0]);
    ...
    public function edit()
    {
        ...
        if ((int) $bookID) {
            $buku = $this->buku_m->get_single_book(array('id_buku' => $bookID, 'dihapus_pada' => 0));
            if (calculate($buku)) {
                $this->data['headerassets'] = array(
                    'css'      => array(
                        'assets/plugins/bootstrap-datepicker/dist/css/bootstrap-datepicker.min.css',
                ...
                if ($_POST) {
                    ...
                    if ($this->form_validation->run() == false) {
                        ...
                        if ($array['jumlah'] >= $buku->jumlah) {
                            $bookitems           = pluck($this->itembuku_m->get_order_by_bookitem(['id_buku' => $bookID]), 'bookitemID', 'bookno');
                            $insertbookitemArray = [];
                            $updatebookitemArray = [];
                            ...
                            if (calculate($insertbookitemArray)) {
                                $this->itembuku_m->insert_bookitem_batch($insertbookitemArray);
                            }
                            if (calculate($updatebookitemArray)) {
                                $this->itembuku_m->update_bookitem_batch($updatebookitemArray, 'bookitemID');
                            }
                        } else {
                            $bookitems = pluck($this->itembuku_m->get_order_by_bookitem(array('id_buku' => $bookID)), 'bookitemID', 'bookno');
                            ...
                            $this->itembuku_m->update_bookitem_batch($deletebookitemArray, 'bookitemID');
                        }
                        $this->buku_m->update_book($array, $bookID);
                        $this->session->set_flashdata('success', 'Success');
                        redirect(base_url('buku/index'));
                    }
                } else {
    ...
    public function view()
    {
        ...
        if ((int) $bookID) {
            $buku = $this->buku_m->get_single_book(array('id_buku' => $bookID));
            if (calculate($buku)) {
                $this->data['buku'] = $buku;

                $this->data['kategori_buku'] = [];
                if ((int) $buku->bookcategoryID) {
                    $this->data['kategori_buku'] = $this->kategoribuku_m->get_single_bookcategory(['bookcategoryID' => $buku->bookcategoryID]);
                }
                $this->data['rak'] = [];
                if ((int) $buku->rackID) {
                    $this->data['rak'] = $this->rak_m->get_single_rack(['rackID' => $buku->rackID]);
                }
    ...
    public function delete()
    {
        $bookID = htmlentities(escapeString($this->uri->segment(3)));
        if ((int) $bookID) {
            $buku = $this->buku_m->get_single_book(array('id_buku' => $bookID, 'deleted_at !=' => 1));
            if (calculate($buku)) {
                $this->buku_m->update_book(['dihapus_pada' => 1], $bookID);
                $this->itembuku_m->update_bookitem_by_bookID(['dihapus_pada' => 1], $bookID);
                $this->session->set_flashdata('success', 'Success');
                redirect(base_url('buku/index'));
            } else {
                $this->data["subview"] = "_not_found";
