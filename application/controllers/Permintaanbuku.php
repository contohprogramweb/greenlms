...
class Permintaanbuku extends Admin_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('Permintaanbuku_m');
    ...
    public function index()
    {
        ...
        $this->data['requestbooks']  = $this->permintaanbuku_m->get_requestbook();
        $this->data['bookcategorys'] = pluck($this->kategoribuku_m->get_bookcategory(), 'nama', 'bookcategoryID');
    ...
    public function add()
    {
        ...
        $this->data['bookcategorys'] = $this->kategoribuku_m->get_order_by_bookcategory(array('status' => 1));
        if ($_POST) {
            $rules = $this->rules();
            ...
            if ($this->form_validation->run() == false) {
                ...
                $this->permintaanbuku_m->insert_requestbook($array);
    ...
    public function edit()
    {
        ...
        if ((int) $requestbookID) {
            $permintaan_buku = $this->permintaanbuku_m->get_single_requestbook(['requestbookID' => $requestbookID, 'status' => 0, 'dihapus_pada' => 0]);
            if (calculate($permintaan_buku)) {
                ...
                if ($_POST) {
                    ...
                    if ($this->form_validation->run() == false) {
                        ...
                        $this->permintaanbuku_m->update_requestbook($array, $requestbookID);
                        $this->session->set_flashdata('success', 'Success');
                        redirect(base_url('permintaan_buku/index'));
                    }
                } else {
    ...
    public function view()
    {
        ...
        if ((int) $requestbookID) {
            $permintaan_buku = $this->permintaanbuku_m->get_single_requestbook(array('requestbookID' => $requestbookID));
            if (calculate($permintaan_buku)) {
                $this->data['permintaan_buku']  = $permintaan_buku;
                $this->data['kategori_buku'] = $this->kategoribuku_m->get_single_bookcategory(array('bookcategoryID' => $permintaan_buku->bookcategoryID));
    ...
    public function delete()
    {
        $requestbookID = htmlentities(escapeString($this->uri->segment(3)));
        if ((int) $requestbookID) {
            $permintaan_buku = $this->permintaanbuku_m->get_single_requestbook(array('requestbookID' => $requestbookID, 'dihapus_pada' => 0));
            if (calculate($permintaan_buku)) {
                $this->permintaanbuku_m->update_requestbook(['dihapus_pada' => 1], $requestbookID);
                $this->session->set_flashdata('success', 'Success');
                redirect(base_url('permintaan_buku/index'));
            } else {
                $this->data["subview"] = "_not_found";
    ...
    public function rejected()
    {
        ...
        if (permissionChecker('requestbook_delete') && (int) $requestbookID) {
            $permintaan_buku = $this->permintaanbuku_m->get_single_requestbook(array('requestbookID' => $requestbookID, 'status' => 0, 'dihapus_pada' => 0));
            if (calculate($permintaan_buku)) {
                $this->permintaanbuku_m->update_requestbook(['status' => 2], $requestbookID);
                $this->session->set_flashdata('success', 'Success');
                redirect(base_url('permintaan_buku/index'));
            } else {
                $this->data["subview"] = "_not_found";
            ...
            if (calculate($permintaan_buku) && ($permintaan_buku->status == 0) && ($permintaan_buku->dihapus_pada == 0)) {
                ...
                if ($_POST) {
                    ...
                    if ($this->form_validation->run() == false) {
                        ...
                        $this->itembuku_m->insert_bookitem_batch($bookitemArray);

                        $this->permintaanbuku_m->update_requestbook(['status' => 1], $requestbookID);
