...
class Masuk extends Admin_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('Aturulangikatasandi_m');
    ...
    public function index()
    {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() == false) {
                ...
                $this->load->view('masuk/index', $this->data);
            } else {
                $array['username_or_email'] = $this->input->post('username_or_email');
                $array['kata_sandi']          = $this->password_hash($this->input->post('kata_sandi'));

                $anggota = $this->masuk_m->get_single_login_by_username_or_email_and_password($array);
                if (calculate($anggota) && $anggota->status == 1 && $anggota->id_peran != 4) {
                    $peran                          = $this->peran_m->get_single_role(array('id_peran' => $anggota->id_peran));
                    $sessionArray                  = [];
    ...
    public function reset_kata_sandi()
    {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() == false) {
                $this->data['errors'] = $this->form_validation->error_array();
                $this->load->view('masuk/reset_kata_sandi', $this->data);
            } else {
                $username_or_email = $this->input->post('username_or_email');
                $anggota            = $this->masuk_m->get_single_login_check_by_username_or_email($username_or_email);
                if (calculate($anggota)) {
                    ...
                    $this->aturulangikatasandi_m->insert_resetpassword($resetArray);
    ...
    public function registermember()
    {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() == false) {
                $this->data['errors'] = $this->form_validation->error_array();
                $this->load->view('masuk/registermember', $this->data);
            } else {
                $array                = [];
                ...
                $this->masuk_m->insert_login($array);
    ...
    public function resetpasswordconfirm()
    {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() == false) {
                $this->data['errors'] = $this->form_validation->error_array();
                $this->load->view('masuk/resetpasswordconfirm', $this->data);
            } else {
                $anggota = $this->masuk_m->get_single_login_check_by_username_or_email($this->input->post('username_or_email'));
                if (calculate($anggota)) {
                    $resetArray                      = [];
                    $resetArray['username_or_email'] = $this->input->post('username_or_email');
                    $resetArray['kode']              = $this->input->post('verification_code');

                    $valid_members = $this->aturulangikatasandi_m->get_single_resetpassword_by_username_or_email_and_code($resetArray);
                    if (calculate($valid_members)) {
                        ...
                        if ($password == $confirm_password) {
                            ...
                            $this->anggota_m->update_member($array, $anggota->id_anggota);
    ...
    {
        $anggota = $this->masuk_m->get_single_login(array('surel' => $email));
        if (calculate($anggota)) {
