...
class Bukuissue extends Admin_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('Riwayatketerlambatan_m');
        $this->load->model('Konfigurasiperpustakaan_m');
        $this->load->model('Pembayarandandiskon_m');
    ...
    public function index()
    {
        ...
        $this->data['bookissues'] = $this->peminjamanbuku_m->get_order_by_bookissue($issueArray);

        $this->data['id_anggota']     = $memberID;
        $this->data['roles']        = pluck($this->peran_m->get_role(), 'peran', 'id_peran');
        $this->data['members']      = pluck($this->anggota_m->get_member(), 'nama', 'id_anggota');
    ...
    public function add()
    {
        ...
        $this->data['members'] = $this->anggota_m->get_order_by_member(['status' => 1, 'dihapus_pada' => 0], array('id_anggota', 'nama'));
        $this->data['books']   = $this->buku_m->get_order_by_book(array('status' => 0, 'dihapus_pada' => 0));

        $this->data['bookitems'] = [];
        $bookID                  = $this->input->post('id_buku');
        if ((int) $bookID) {
            $this->data['bookitems'] = $this->itembuku_m->get_order_by_bookitem(array('id_buku' => $bookID, 'status' => 0, 'dihapus_pada' => 0), array('bookno'));
        }
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() == false) {
                ...
                $memberID = $this->input->post('id_anggota');
                $anggota   = $this->anggota_m->get_single_member(['id_anggota' => $memberID]);
                ...
                if (!calculate($konfigurasi_perpustakaan)) {
                    $konfigurasi_perpustakaan = (object) $this->konfigurasiperpustakaan_m->konfigurasi_perpustakaan;
                }
                ...
                $bookissueID                = $this->peminjamanbuku_m->insert_bookissue($array);

                $item_buku = $this->itembuku_m->get_single_bookitem(['id_buku' => $array['id_buku'], 'bookno' => $array['bookno'], 'status' => 0, 'dihapus_pada' => 0]);
                if (calculate($item_buku)) {
                    $this->itembuku_m->update_bookitem(['status' => 1], $item_buku->bookitemID);
                }
                $item_buku = $this->itembuku_m->get_order_by_bookitem(['id_buku' => $array['id_buku'], 'status' => 0, 'dihapus_pada' => 0]);
                if (!calculate($item_buku)) {
                    $this->buku_m->update_book(['status' => 1], $array['id_buku']);
                }
                ...
                $this->riwayatketerlambatan_m->insert_finehistory($fineArray);
    ...
    public function edit()
    {
        ...
        if ((int) $bookissueID) {
            $peminjaman_buku = $this->peminjamanbuku_m->get_single_bookissue(array('bookissueID' => $bookissueID, 'dihapus_pada' => 0, 'status' => 0, 'diperbarui' => 1));
                ...
                if ($_POST) {
                    ...
                    if ($this->form_validation->run() == false) {
                        ...
                        if (($peminjaman_buku->id_buku != $array['id_buku']) || ($peminjaman_buku->bookno != $array['bookno'])) {
                            $item_buku = $this->itembuku_m->get_single_bookitem(['id_buku' => $peminjaman_buku->id_buku, 'bookno' => $peminjaman_buku->bookno, 'status' => 1, 'dihapus_pada' => 0]);
                            if (calculate($item_buku)) {
                                $this->itembuku_m->update_bookitem(['status' => 0], $item_buku->bookitemID);
                            }
                        }
                        ...
                        $item_buku = $this->itembuku_m->get_single_bookitem(['id_buku' => $array['id_buku'], 'status' => 0, 'dihapus_pada' => 0]);
                        if (!calculate($item_buku)) {
                            $this->buku_m->update_book(['status' => 1], $array['id_buku']);
                        }

                        $this->peminjamanbuku_m->update_bookissue($array, $bookissueID);
                        $this->session->set_flashdata('success', 'Success');
                        redirect(base_url('peminjaman_buku/index'));
                    }
                } else {
    ...
    public function view()
    {
        ...
        if ((int) $bookissueID) {
            ...
            $issueArray['dihapus_pada']  = 0;
            $peminjaman_buku                 = $this->peminjamanbuku_m->get_single_bookissue($issueArray);
            if (calculate($peminjaman_buku)) {
                $this->data['peminjaman_buku'] = $peminjaman_buku;
                $this->data['anggota']    = $this->anggota_m->get_single_member(array('id_anggota' => $peminjaman_buku->id_anggota));
                $this->data['peran']      = $this->peran_m->get_single_role(array('id_peran' => $peminjaman_buku->id_peran));
                $this->data['buku']      = $this->buku_m->get_single_book(array('id_buku' => $peminjaman_buku->id_buku));
                $this->db->order_by('finehistoryID desc');
                $this->data['riwayat_denda']         = $this->riwayatketerlambatan_m->get_order_by_finehistory(array('bookissueID' => $bookissueID));
                $this->data['paymentanddiscounts'] = $this->pembayarandandiskon_m->get_order_by_paymentanddiscount(array('bookissueID' => $bookissueID));
    ...
    public function delete()
    {
        ...
        if ((int) $bookissueID) {
            $peminjaman_buku = $this->peminjamanbuku_m->get_single_bookissue(array('bookissueID' => $bookissueID, 'dihapus_pada' => 0, 'status' => 0, 'diperbarui' => 1));
            if (calculate($peminjaman_buku)) {
                $item_buku = $this->itembuku_m->get_single_bookitem(['id_buku' => $peminjaman_buku->id_buku, 'bookno' => $peminjaman_buku->bookno, 'status !=' => 2, 'dihapus_pada' => 0]);
                if (calculate($item_buku)) {
                    $this->itembuku_m->update_bookitem(['status' => 0], $item_buku->bookitemID);
                    $this->buku_m->update_book(['status' => 0], $array->id_buku);
                }
                $this->peminjamanbuku_m->update_bookissue(['dihapus_pada' => 1], $bookissueID);
                $this->session->set_flashdata('success', 'Success');
                redirect(base_url('peminjaman_buku/index'));
            } else {
                $this->data["subview"] = "_not_found";
            ...
            if (calculate($peminjaman_buku)) {
                ...
                if ($_POST) {
                    ...
                    if ($this->form_validation->run() == false) {
                        ...
                        $this->riwayatketerlambatan_m->insert_finehistory($array);

                        $roleID           = $peminjaman_buku->id_peran;
                        $konfigurasi_perpustakaan = $this->konfigurasiperpustakaan_m->get_single_libraryconfigure(array('id_peran' => $roleID));
                        if (!calculate($konfigurasi_perpustakaan)) {
                            $konfigurasi_perpustakaan = $this->konfigurasiperpustakaan_m->konfigurasi_perpustakaan;
                        }
                        ...
                        $this->peminjamanbuku_m->update_bookissue($issueArray, $bookissueID);

                        if (($bookstatusID == 2) || ($bookstatusID == 3)) {
                            $item_buku = $this->itembuku_m->get_single_bookitem(['id_buku' => $peminjaman_buku->id_buku, 'bookno' => $peminjaman_buku->bookno, 'dihapus_pada' => 0]);
                            if (calculate($item_buku)) {
                                $this->itembuku_m->update_bookitem(['status' => 0], $item_buku->bookitemID);
                                $this->buku_m->update_book(['status' => 0], $peminjaman_buku->id_buku);
                            }
                        }
    ...
    public function get_fineamount()
    {
        ...
        if ($_POST && permissionChecker('bookissue_view')) {
            ...
            if ((int) $bookissueID) {
                $peminjaman_buku = $this->peminjamanbuku_m->get_single_bookissue(['bookissueID' => $bookissueID]);
                if (calculate($peminjaman_buku)) {
                    ...
                    if ($max_renewed_limit > calculate($peminjaman_buku->diperbarui)) {
                        ...
                        if ($bookstatusID == 3) {
                            $buku       = $this->buku_m->get_single_book(['id_buku' => $peminjaman_buku->id_buku]);
    ...
    public function set_paymentamount()
    {
        ...
        if ($_POST && permissionChecker('bookissue_add')) {
            ...
            if ($this->form_validation->run() == false) {
                ...
                $this->pembayarandandiskon_m->insert_paymentanddiscount($array);
    ...
    public function get_paymentamount()
    {
        ...
        if ($_POST && permissionChecker('bookissue_add')) {
            $bookissueID = $this->input->post('bookissueID');
            if ((int) $bookissueID) {
                $peminjaman_buku = $this->peminjamanbuku_m->get_single_bookissue(['bookissueID' => $bookissueID], array('fineamount', 'discountamount', 'paymentamount'));
                if (calculate($peminjaman_buku)) {
                    $retArray['status']        = true;
    ...
    public function get_book_item()
    {
        ...
        if ($_POST && permissionChecker('bookissue_add')) {
            $bookID = $this->input->post('id_buku');
            if ((int) $bookID) {
                $bookitems = $this->itembuku_m->get_order_by_bookitem(array('id_buku' => $bookID, 'status' => 0, 'dihapus_pada' => 0), array('bookno'));
    ...
    public function check_eligible_for_book($memberID)
    {
        ...
        if ((int) $memberID && (int) $roleID) {
            ...
            $peminjaman_buku         = $this->peminjamanbuku_m->get_order_by_bookissue(['id_anggota' => $memberID, 'status' => 0, 'dihapus_pada' => 0]);
    ...
    public function check_available_book($bookno)
    {
        ...
        if ((int) $bookno && (int) $bookID) {
            ...
            $bookissueID = htmlentities(escapeString($this->uri->segment(3)));
            if ((int) $bookissueID) {
                $peminjaman_buku = $this->peminjamanbuku_m->get_single_bookissue(array('bookissueID' => $bookissueID, 'dihapus_pada' => 0));
                if (($bookID == $peminjaman_buku->id_buku) && ($bookno == $peminjaman_buku->bookno)) {
                    $f = true;
                } else {
                    $item_buku = $this->itembuku_m->get_order_by_bookitem(['id_buku' => $bookID, 'bookno' => $bookno, 'status' => 0, 'dihapus_pada' => 0]);
                    if (calculate($item_buku)) {
                        $f = true;
                    }
                }
    ...
    public function check_payment_amount()
    {
        ...
        if ($_POST && permissionChecker('bookissue_add') && (int) $bookissueID) {
            if (empty($paymentamount)) {
                ...
                $peminjaman_buku = $this->peminjamanbuku_m->get_single_bookissue(['bookissueID' => $bookissueID, 'paidstatus !=' => 2]);
                if (calculate($peminjaman_buku)) {
                    $totalfineamount       = $peminjaman_buku->fineamount;
    ...
    public function check_renew_eligible()
    {
        $bookissueID = htmlentities(escapeString($this->uri->segment(3)));
        if ((int) $bookissueID) {
            $peminjaman_buku = $this->peminjamanbuku_m->get_single_bookissue(['bookissueID' => $bookissueID, 'status' => 0, 'dihapus_pada' => 0]);
