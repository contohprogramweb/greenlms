...
class Bukuissuereport extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        $this->data['roles']         = pluck($this->peran_m->get_role(), 'obj', 'id_peran');
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() == false) {
                ...
                $this->data["subview"] = "laporan/peminjaman_buku/index";
    ...
    public function pdf()
    {
        ...
        if (((int) $bookcategoryID || $bookcategoryID == 0) && ((int) $bookID || $bookID == 0) && ((int) $roleID || $roleID == 0) && ((int) $memberID || $memberID == 0) && ((int) $status || $status == 0) && ((int) $fromdate || $fromdate == 0) && ((int) $todate || $todate == 0)) {
            ...
            $this->pdf->create(['stylesheet' => 'bookissuereport.css', 'view' => 'laporan/peminjaman_buku/pdf.php', 'data' => $this->data]);
        } else {
    ...
    private function _queryArray($qArray)
    {
        ...
        $bookissues = [];
        if (calculate($userArray)) {
            $bookissues = $this->peminjamanbuku_m->get_order_by_bookissue_for_bookissuereport($queryArray);
        }
        ...
        $this->data['books']          = pluck($this->buku_m->get_book(), 'nama', 'id_buku');
