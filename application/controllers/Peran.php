...
class Peran extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        $this->data['roles'] = $this->peran_m->get_role(array('id_peran', 'peran', 'tanggal_dibuat'));
    ...
    public function add()
    {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() == false) {
                ...
                $this->peran_m->insert_role($array);
    ...
    public function edit()
    {
        ...
        if ((int) $roleID) {
            $this->data['peran'] = $this->peran_m->get_single_role($roleID);
            if (calculate($this->data['peran'])) {
                if ($_POST) {
                    ...
                    if ($this->form_validation->run() == false) {
                        ...
                        $this->peran_m->update_role($array, $roleID);
                        $this->session->set_flashdata('success', 'Success');
                        redirect(base_url('peran/index'));
                    }
                } else {
    ...
    public function delete()
    {
        $roleID = escapeString($this->uri->segment('3'));
        if ((int) $roleID) {
            $peran = $this->peran_m->get_single_role(array('id_peran' => $roleID));
            if (calculate($peran)) {
                if (!in_array($roleID, $this->notdeleteArray)) {
                    $this->peran_m->delete_role($roleID);
                    $this->session->set_flashdata('success', 'Success');
                } else {
    ...
    public function check_unique_role($peran)
    {
        ...
        if ((int) $roleID) {
            $peran = $this->peran_m->get_single_role(array('peran' => $peran, 'roleID !=' => $roleID));
            if (calculate($peran)) {
            ...
            $peran = $this->peran_m->get_single_role(array('peran' => $peran));
            if (calculate($peran)) {
                $this->form_validation->set_message("check_unique_role", "The %s is already exits.");
                return false;
            }
