...
class Bukuelektronik extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        $config['total_rows']     = calculate($this->bukuelektronik_m->get_ebook());
        $config['per_page']       = 12;
        $config['num_links']      = 1;
    ...
    public function add()
    {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() == false) {
                ...
                $this->bukuelektronik_m->insert_ebook($array);
            ...
            if (calculate($this->data['buku_elektronik'])) {
                if ($_POST) {
                    ...
                    if ($this->form_validation->run() == false) {
                        ...
                        $this->bukuelektronik_m->update_ebook($array, $ebookID);
                        $this->session->set_flashdata('success', 'Success');
                        redirect(base_url('buku_elektronik/index'));
                    }
                } else {
    ...
    public function delete()
    {
        ...
        if ((int) $ebookID) {
            ...
            if (calculate($buku_elektronik)) {
                ...
                $this->bukuelektronik_m->delete_ebook($ebookID);
                $this->session->set_flashdata('success', 'Success');
                redirect(base_url('buku_elektronik/index'));
            } else {
                $this->data["subview"] = "_not_found";
    ...
    public function check_unique_ebook($name)
    {
        $ebookID = htmlentities(escapeString($this->uri->segment(3)));
        if ((int) $ebookID) {
            $buku_elektronik = $this->bukuelektronik_m->get_single_ebook(array('nama' => $name, 'ebookID !=' => $ebookID));
            if (calculate($buku_elektronik)) {
            ...
            $buku_elektronik = $this->bukuelektronik_m->get_single_ebook(array('nama' => $name));
            if (calculate($buku_elektronik)) {
                $this->form_validation->set_message("check_unique_ebook", "The %s is already exits.");
                return false;
            }
