...
class Dasbor extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        $this->data['members'] = pluck($this->anggota_m->get_member(), 'obj', 'id_anggota');

        $this->db->order_by('chatID desc');
        $chats = $this->obrolan_m->get_chat_by_limit(20);
        asort($chats);

        $this->_incomeAndExpense();

        $this->data['chats']   = $chats;
        $this->data["subview"] = "dasbor/index";
    ...
    private function _incomeAndExpense()
    {
        ...
        $incomes  = $this->pemasukan_m->get_order_by_income_for_report($querArray);
        $expenses = $this->pengeluaran_m->get_order_by_expense_for_report($querArray);
    ...
    public function obrolan()
    {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() == false) {
                ...
                $result         = $this->obrolan_m->insert_chat($array);
    ...
    public function getchatmessage()
    {
        ...
        if ($_POST) {
            ...
            if ((int) $chatID) {
                ...
                $members = pluck($this->anggota_m->get_member(), 'obj', 'id_anggota');
                $chats   = $this->obrolan_m->get_chat_by_limit($showitem, $chatID);

                $retArray['data'] = $this->load->view('dasbor/getchat', array('chats' => $chats, 'members' => $members), true);
                if ($retArray['data'] != '') {
                    $retArray['status'] = true;
                }
            }
