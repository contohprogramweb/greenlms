...
class Catatanizin extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        $this->data['permissionlogs'] = $this->catatanizin_m->get_permissionlog();
    ...
    public function add()
    {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() == false) {
                ...
                $this->catatanizin_m->insert_permissionlog($array);
    ...
    public function edit()
    {
        ...
        if ((int) $permissionlogID) {
            $this->data['catatan_izin'] = $this->catatanizin_m->get_single_permissionlog($permissionlogID);
            if (calculate($this->data['catatan_izin'])) {
                if ($_POST) {
                    ...
                    if ($this->form_validation->run() == false) {
                        ...
                        $this->catatanizin_m->update_permissionlog($array, $permissionlogID);
                        $this->session->set_flashdata('success', 'Success');
                        redirect(base_url('catatan_izin/index'));
                    }
                } else {
    ...
    public function delete()
    {
        $permissionlogID = escapeString($this->uri->segment('3'));
        if ((int) $permissionlogID) {
            $catatan_izin = $this->catatanizin_m->get_single_permissionlog($permissionlogID);
            if (calculate($catatan_izin)) {
                $this->catatanizin_m->delete_permissionlog($permissionlogID);
                $this->session->set_flashdata('success', 'Success');
                redirect(base_url('catatan_izin/index'));
            } else {
                $this->data["subview"] = "_not_found";
    ...
    public function check_unique_name($name)
    {
        ...
        if ((int) $permissionlogID) {
            $anggota = $this->catatanizin_m->get_single_permissionlog(array('nama' => $name, 'permissionlogID !=' => $permissionlogID));
            ...
            $anggota = $this->catatanizin_m->get_single_permissionlog(array('nama' => $name));
            if (calculate($anggota)) {
                $this->form_validation->set_message("check_unique_name", "The %s is already exits.");
                return false;
            }
